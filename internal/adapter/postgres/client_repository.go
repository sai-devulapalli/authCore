...
func (r *ClientRepository) Create(ctx context.Context, c client.Client) error {
	...
	defer cancel()
	// id UUID is auto-generated by the DB; client_id holds the OAuth client identifier.
	query := `INSERT INTO clients (client_id, tenant_id, client_name, client_type, secret_hash, redirect_uris, allowed_scopes, allowed_grant_types, created_at, updated_at)
...
func (r *ClientRepository) Update(ctx context.Context, c client.Client) error {
	...
	query := `UPDATE clients SET client_name = $1, redirect_uris = $2, allowed_scopes = $3, updated_at = $4
		WHERE client_id = $5 AND tenant_id = $6 AND deleted_at IS NULL`
...
func (r *ClientRepository) Delete(ctx context.Context, id, tenantID string) error {
	ctx, cancel := WithQueryTimeout(ctx)
	defer cancel()
	query := `UPDATE clients SET deleted_at = $1 WHERE client_id = $2 AND tenant_id = $3 AND deleted_at IS NULL`
...
func (r *ClientRepository) UpdateAPIKey(ctx context.Context, clientID, tenantID string, apiKeyHash []byte) error {
	ctx, cancel := WithQueryTimeout(ctx)
	defer cancel()
	query := `UPDATE clients SET api_key_hash = $1, updated_at = $2 WHERE client_id = $3 AND tenant_id = $4 AND deleted_at IS NULL`
