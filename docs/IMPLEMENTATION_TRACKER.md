...
| CareOS BFF integration — Optional tenant middleware, cross-tenant JWKS, refresh-token tenant resolution, issuer from tenant record | — | CareOS integration | Done |
| Deterministic client provisioning (fixed `client_id` + `client_secret` fields on `CreateClientRequest`) | — | CareOS integration | Done |
| UUID primary keys + `VARCHAR(n)` bounded types across all 19 migrations | — | Schema hygiene | Done |
| `clients` table: separate internal `UUID id` from OAuth `client_id VARCHAR(100)` | — | Schema hygiene | Done |
| Dockerfile: `alpine:3.20` runtime (replaces distroless; adds wget for healthchecks) | — | Infra fix | Done |
...
| 013 | `013_add_token_version.sql` | users, tenants, clients (ALTER) | GTM |
| 014 | `014_create_admin_users.sql` | admin_users | GTM |
| 015 | `015_enable_rls.sql` | RLS policies on 12 tables | GTM |
| 016 | `016_audit_immutability.sql` | Audit append-only rules | GTM |
| 017 | `017_add_foreign_keys.sql` | FK constraints | GTM |
| 018 | `018_add_tenant_settings.sql` | tenants (ALTER) | GTM |
| 019 | `019_create_webhooks.sql` | webhooks | GTM |
...
| 2026-04-05 | **Schema:** All internal `id` columns TEXT → `UUID PRIMARY KEY DEFAULT gen_random_uuid()` across all 19 migrations |
| 2026-04-05 | **Schema:** All FK columns (tenant_id, subject, role_id, etc.) TEXT → `UUID` |
| 2026-04-05 | **Schema:** `clients` table split: `id UUID PK` (internal, auto-generated) + `client_id VARCHAR(100) UNIQUE` (OAuth identifier) |
| 2026-04-05 | **Schema:** Bounded strings use VARCHAR(n): email→254, name→200, algorithm→10, ip→45, phone→30, action→100, etc. TEXT kept for URLs, token strings, user agents |
| 2026-04-05 | **Schema:** RLS policies cast `nullif(current_setting('app.tenant_id',true),'')::uuid` to fix UUID=TEXT operator error |
| 2026-04-05 | **Go:** All `generateID()`/`mustGenerateID()` across 8 services → `uuid.New().String()` (was base64url of 16 random bytes) |
| 2026-04-05 | **Go:** `tenant_repository`: `active_key_id` passed as `*string` (nil for empty) and scanned as `*string` to handle nullable UUID column |
| 2026-04-05 | **Go:** `client_repository`: INSERT uses `client_id` column; all WHERE clauses query `client_id`; internal `id` UUID auto-generated by DB |
| 2026-04-05 | **Go:** `jwk_repository.GetAllPublic()`: returns all active keys across all tenants when `tenantID` is empty — enables cross-tenant JWKS for BFF backends |
| 2026-04-05 | **Go:** `auth/service`: issuer resolved from `tenantRepo.GetByID()` at token issuance; was hardcoded `"https://authplex"` |
| 2026-04-05 | **Go:** `auth/service`: added `GetRefreshTokenTenantID()` — resolves tenant from stored refresh token for grants sent without `X-Tenant-ID` header |
| 2026-04-05 | **Go:** `client/service`: `CreateClientRequest` accepts optional `client_id` + `client_secret` — enables deterministic provisioning via init scripts |
| 2026-04-05 | **Go:** `middleware/tenant`: added `Optional()` — missing tenant header passes through instead of 400, for endpoints that self-resolve tenant |
| 2026-04-05 | **Go:** `handler/token`: pre-resolves `tenantID` from stored refresh token for `refresh_token` grants; `/token` and `/revoke` use `tenantResolver.Optional()` |
| 2026-04-05 | **Go:** `main.go`: wired `tenantRepo` + `userRepo` into `authSvc` (were nil, so issuer fix and token versioning never executed) |
| 2026-04-05 | **Infra:** Dockerfile final stage `gcr.io/distroless/static-debian12` → `alpine:3.20` + wget (distroless has no shell/wget, healthchecks were failing) |
